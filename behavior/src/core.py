...
def get_task_trials(
    ...
    for sess in sessions:
        ...
        if only_correct and not set(required_cols).issubset(df.columns):
            continue
        ...
        filtered.append({task_key: df_out})
...
def merge_trials_with_df_idx(
    ...
    Modified to explicitly keep the 'trial_col' (e.g., 'trial') in the output.
    """
    ...
    # Drop only the columns ending with '_drop' (which came from df_trials)
    # The original join column 'trial_col' (e.g., 'trial') is explicitly kept.
...
def add_memory_delay_features(merged: pd.DataFrame) -> pd.DataFrame:
    ...
    df["memoryDelay"] = pd.to_numeric(df["fixptOff"], errors="coerce") - pd.to_numeric(df["targetOff"], errors="coerce")

    # Current, previous, and response angles
    df["curr"] = pd.to_numeric(df["targAng"], errors="coerce")
    df["resp"] = pd.to_numeric(df["respAng_dva"], errors="coerce")
    df["prev"] = df["curr"].shift(1)

    # Vectorized circular ops
    curr = df["curr"].to_numpy(dtype=float)
    prev = df["prev"].to_numpy(dtype=float)
    resp = df["resp"].to_numpy(dtype=float)
...
def _load_h5_group(group: h5py.Group) -> dict:
    """Recursively load HDF5 group into nested dict, dereferencing object references."""
    data = {}
    for key in group.keys():
        item = group[key]
        if isinstance(item, h5py.Dataset):
            arr = item[()]
            # Dereference object references if present
            if arr.dtype == object:
                try:
                    arr = np.array([group.file[ref][()] if isinstance(ref, h5py.Reference) else ref for ref in arr.flat]).reshape(arr.shape)
                except Exception:
                    pass  # Keep original if dereferencing fails
            data[key] = arr
        elif isinstance(item, h5py.Group):
            data[key] = _load_h5_group(item)
    return data

def load_sessions_v73(session_ids: list, eye_dir: str = None) -> Dict[str, Dict[str, any]]:
    """
    Load multiple v7.3 MAT files for eye-tracker data.
    Handles HDF5 object references in ainSchema.
    
    Parameters
    ----------
    session_ids : list
        List of session IDs to load
    eye_dir : str, optional
        Eye tracker directory (default uses standard path)
    
    Returns
    -------
    mat_data : dict
        {session_id: {'ain': array, 'ainSchema': ..., ...}, ...}
        Format matches v7.2 output: mat_data[session]['ain'].shape = (6001, 4, n_trials)
    """
    if eye_dir is None:
    ...
    for session_id in session_ids:
        filepath = Path(eye_dir) / f"{session_id}.mat"
        ...
        try:
            ...
            # Dereference ainSchema if it contains HDF5 references
            if 'ainSchema' in data and isinstance(data['ainSchema'], dict):
                schema = data['ainSchema']
                if 'index' in schema and isinstance(schema['index'], np.ndarray):
                    # Dereference the index array
                    idx_arr = schema['index']
                    if idx_arr.dtype == object:
                        dereferenced = []
                        with h5py.File(filepath, 'r') as f:
                            for ref in idx_arr.flat:
                                if isinstance(ref, h5py.Reference):
                                    dereferenced.append(f[ref][()])
                                else:
                                    dereferenced.append(ref)
                        schema['index'] = np.array(dereferenced, dtype=object).reshape(idx_arr.shape)
            
            # Transpose ain to match v7.2 format: (6001, 4, n_trials) instead of (n_trials, 4, 6001)
            if 'ain' in data:
                data['ain'] = np.transpose(data['ain'], (2, 1, 0))
            
            mat_data[session_id] = data
            print(f"✓ Loaded {session_id}: ain shape {data['ain'].shape}")
            
        except FileNotFoundError:
            print(f"✗ File not found: {session_id}")
        except Exception as e:
